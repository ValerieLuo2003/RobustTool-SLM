...
def aggregate_metrics(records: Iterable[Mapping[str, Any]]) -> dict[str, MetricValue]:
    ...
    answer_semantic_items = [
        item for item in items if item.get("final_answer_semantic_eligible", 0)
    ]
    return {
        ...
        "final_answer_semantic_accuracy": MetricValue.rate(
            sum(item.get("final_answer_semantic_correct", 0) for item in answer_semantic_items),
            len(answer_semantic_items),
        ),
